...
class MainWindow(QMainWindow):
    ...
    def _setup_dock_panels(self):
        ...
        # Histogram dock — alt
        self._histogram = HistogramPanel()
        self._histogram.apply_levels.connect(self._apply_levels)

        hist_dock = QDockWidget("Histogram / Seviyeler", self)
    ...
    def _apply_levels(self, black_in: float, gamma: float, white_in: float,
                       black_out: float, white_out: float):
        """
        Histogram panelinden gelen Levels/Seviyeleri görüntüye uygular.
        Photoshop Levels mantığı:
          1. Giriş kırpma: [black_in, white_in] → [0, 1]
          2. Gamma düzeltmesi
          3. Çıkış ölçekleme: [0,1] → [black_out, white_out]
        """
        if self._current_image is None:
            return

        img = self._current_image.astype(np.float32)

        # 1. Giriş normalizasyonu
        rng_in = float(white_in - black_in)
        if rng_in < 1e-9:
            rng_in = 1e-9
        normalized = np.clip((img - black_in) / rng_in, 0.0, 1.0)

        # 2. Gamma düzeltmesi (gamma != 1 ise)
        if abs(gamma - 1.0) > 1e-4:
            # Gamma > 1 → açık, Gamma < 1 → koyu (Photoshop convention)
            exp = 1.0 / float(gamma)
            normalized = np.power(np.clip(normalized, 1e-9, 1.0), exp).astype(np.float32)

        # 3. Çıkış ölçekleme
        rng_out = float(white_out - black_out)
        result = np.clip(black_out + normalized * rng_out, 0.0, 1.0).astype(np.float32)

        self._set_image(result, label=f"Seviyeler ({black_in:.3f}→{white_in:.3f} γ{gamma:.2f})")
        self._status("✓ Seviyeler uygulandı")
